...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (doc.exists) {
        ...
        // মালিকের আইডি ও টাইটেল সংরক্ষণ করা হলো
        currentOwnerId = data.userId || data.uid || data.ownerId || data.owner?.uid;
        currentPostTitle = data.title;
    }

    // মেসেজ বাটনে ক্লিক ইভেন্ট (আপনার ডিটেইলস পেজের মেসেজ বাটনের আইডি যদি "messageOwnerButton" হয়)
    document.getElementById('messageOwnerButton')?.addEventListener('click', startChat);
});

// নতুন চ্যাট তৈরি বা পূর্বের চ্যাট ওপেন করার ফাংশন
async function startChat() {
    const currentUser = firebase.auth().currentUser;
    if (!currentUser) {
        alert("চ্যাট করার জন্য দয়া করে লগইন করুন।");
        location.href = "login.html"; // আপনার লগইন পেজের লিংক দিন
        return;
    }
    ...
    try {
        // চেক করুন এই ইউজারের সাথে এই নির্দিষ্ট পোস্টের জন্য আগে চ্যাট আছে কি না
        ...
        if (!existingChat.empty) {
            // চ্যাট পাওয়া গেছে → সেটি ওপেন হবে

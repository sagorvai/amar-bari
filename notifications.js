// notifications.js - সম্পূর্ণ ডায়নামিক ও স্মার্ট নোটিফিকেশন ইঞ্জিন (ডুয়েল-মোড সাপোর্টেড)
...
// 📌 সক্রিয় আইডেন্টিটি বের করার সেন্ট্রাল ফাংশন (ইউজার বা পেজ)
function getActiveIdentityData() {
    ...
    // ব্যাকআপ Fallback (যদি header-sync লোড না থাকে বা লেট হয়)
        ...
        if (activeMode === 'company') {
            ...
            if (savedCompanyId) {
                activeIdentity = {
                    ...
                    type: 'company',
                    ownerUid: user.uid,
                    name: localStorage.getItem('activeCompanyName') || 'কোম্পানি পেজ',
                    avatar: localStorage.getItem('activeCompanyLogo') || ''
                };
            }
        }
...
// 🎯 ২. একটিভ আইডেন্টিটি (ইউজার/পেজ) ফিল্টার করে নোটিফিকেশন লোড
...
// 🎯 ৩. টোকেন সিঙ্ক (ইউজার এবং পেজ/কোম্পানি উভয়ের নির্দিষ্ট আইডি অনুযায়ী টোকেন সেভ করবে)
async function syncGuestTokenToUser(uid, activeIdentity) {
    try {
        ...
        if (currentToken) {
            // ১. ইউজার কালেকশনে সেভ
            await saveTokenToFirestore("users", uid, currentToken);
            
            // ২. পেজ/কোম্পানি মোডে থাকলে কোম্পানির নির্দিষ্ট আইডিতেও টোকেন সিঙ্ক
...
// 🎯 ৪. রিয়েলটাইম লিসেনার (ইউজার আইডি বা পেজ আইডি অনুযায়ী ফিল্টারিং)
function listenForNotifications(activeIdentity) {
    const notificationContainer = document.getElementById("notifications-list");
    if (!notificationContainer) return;

    // ১. পূর্বের রানিং লিসেনার বন্ধ নিশ্চিত করা (যাতে আগের মোডের বা অন্য আইডেন্টিটির ডেটা জমা না থাকে)
    ...
    // শুধুমাত্র সক্রিয় ইউজার আইডি বা কোম্পানি আইডির জন্য নির্দিষ্ট কোয়েরি
    ...
    const handleSnapshot = (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            // 🚨 ফিল্টার ১: নিজের পাঠানো নোটিফিকেশন নিজে দেখবে না
            if (data.senderId && String(data.senderId) === String(targetId)) {
                return;
            }

            // 🚨 ফিল্টার ২: ডুপ্লিকেট নোটিফিকেশন প্রতিরোধ
        ...
        // সময় অনুযায়ী সাজানো (নতুনগুলো আগে থাকবে)
        docsArray.sort((a, b) => {
            const getMillis = (d) => {
                if (!d) return 0;

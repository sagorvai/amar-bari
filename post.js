// post.js - Fixed with Page Mode Support, Client-Side Image Compression, Fast Parallel Uploads & Price Drop Alert
const db = firebase.firestore();
const storage = firebase.storage();
const auth = firebase.auth();

// এডিট মুড ও পেজ সংক্রান্ত গ্লোবাল ভেরিয়েবল
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const BD_GEOGRAPHY = {
        ...
        "খুলনা বিভাগ": {
            "খুলনা": {
                "সিটি কর্পোরেশন": ["খুলনা সদর", "দৌলতপুর", "খালিশপুর", "خانجاهান আলী", "লবণচরা", "হরিণটানা", "আড়ংঘাটা", "সোনাডাঙ্গা"],
            ...
            "মাগুরা": { "উপজেলা": ["মাগুরা সদর", "শ্রীপুর", "محম্মদপুর", "শালিখা"] },
        ...
        "রাজশাহী বিভাগ": {
            "রাজশাহী": {
                ...
                "উপজেলা": ["পবা", "গোদাগাড়ী", "তানোর", "मोहनপুর", "বাগমারা", "দুর্গাপুর", "পুট্টিয়া", "চারঘাট", "বাঘা"]
            },
            ...
            "নওগাঁ": { "উপজেলা": ["নওগাঁ সদর", "রানীনগর", "আত্রাই", "মহাদেবপুর", "बদলাগছী", "পত্নীতলা", "ধামইরহাট", "নিয়ামতপুর", "পোরশা", "সাপাহার", "মান্দা"] },
        ...
        "সিলেট বিভাগ": {
            "সিলেট": {
                "সিটি কর্পোরেশন": ["কোতোয়ালী", "شاہপরান", "এয়ারপোর্ট", "مোগলাবাজার", "দক্ষিণ সুরমা"],
            ...
            "সُنামগঞ্জ": { "উপজেলা": ["সুনামগঞ্জ সদর", "দক্ষিণ সুনামগঞ্জ", "दোয়ারাবাজার", "ছাতক", "জগন্নাথপুর", "দিরাই", "শালল্লা", "ধর্মপাশা", "তাহিরপুর", "বিশ্বম্ভরপুর", "মধ্যনগর"] },
        ...
        "রংপুর বিভাগ": {
            "রংপুর": {
                ...
                "উপজেলা": ["রংপুর সদর", "মিঠাপुकুর", "পীরগঞ্জ", "পীরগাছা", "কাউনিয়া", "গঙ্গাচড়া", "তারাগঞ্জ", "বদরগঞ্জ"]
            },
        ...
        "ময়মনসিংহ বিভাগ": {
            ...
            "নেত্রকোনা": { "উপজেলা": ["নেত্রকোনা সদর", "বারহাট্টা", "কলমাকান্দা", "দুগাপুর", "পূর্বধলা", "مোহনগঞ্জ", "আটপাড়া", "مদন", "খালীযাজুরী", "কেন্দুয়া"] },
    ...
    propertyForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // 🎯 URL Parameter থেকে pageId চেক করা (পেজ মুডের জন্য)
            const urlParams = new URLSearchParams(window.location.search);
            const pageId = urlParams.get('pageId') || sessionStorage.getItem('activePageId') || null;
            ...
            const propertyData = {
                ...
                userId: user.uid,
                pageId: pageId, // 🎯 পেজ আইডি যুক্ত করা হলো
                postedAsPage: !!pageId, // 🎯 পেজ হিসেবে পোস্ট কি না তা ট্র্যাক করা
                status: 'pending',
            ...
            // প্রিভিউ পেজে রিডাইরেক্ট করার সময় pageId ফ্ল্যাগ পাঠানো
            const redirectUrl = pageId ? `preview.html?pageId=${pageId}` : 'preview.html';
    ...
    if (typeof auth !== 'undefined' && auth.onAuthStateChanged) {
        // ইউআরএল (URL) থেকে এডিট আইডি ও পেজ আইডি চেক করা
        const urlParams = new URLSearchParams(window.location.search);
        editPostId = urlParams.get('edit');
        const pageIdFromUrl = urlParams.get('pageId');

        if (pageIdFromUrl) {
            sessionStorage.setItem('activePageId', pageIdFromUrl);
        }
        ...
        auth.onAuthStateChanged(user => {
            ...
            if (user) {
                ...
                // 🎯 পেজ মুডে থাকলে পেজের ইনফো লোড করার অপশনাল লজিক
                const activePageId = urlParams.get('pageId') || sessionStorage.getItem('activePageId');
                if (activePageId) {
                    db.collection('pages').doc(activePageId).get().then(pageDoc => {
                        if (pageDoc.exists) {
                            const pageData = pageDoc.data();
                            if (primaryPhoneInput && pageData.phoneNumber) {
                                primaryPhoneInput.value = pageData.phoneNumber;
                            }
                        }
                    });
                } else {

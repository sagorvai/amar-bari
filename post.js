...
// --- নতুন ফাংশন: হেডার প্রোফাইল লোড করার জন্য (from post (6).js) ---
...
// --- নতুন ফাংশন: লগআউট হ্যান্ডেলার (from post (6).js) ---
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generateSpecificFields(category, type, stagedData = null, stagedMetadata = null) {
        ...
        if (category === 'বিক্রয়') {
            ...
            priceRentHTML += `</select></div>`;

            priceRentHTML += `
                <div class="input-group">
                    <label for="negotiable">দাম আলোচনা সাপেক্ষ?</label>
                    <div class="radio-group">
                        <input type="radio" id="negotiable-yes" name="negotiable" value="হ্যাঁ" ${stagedData?.negotiable === 'হ্যাঁ' ? 'checked' : ''} required><label for="negotiable-yes">হ্যাঁ</label>
                        <input type="radio" id="negotiable-no" name="negotiable" value="না" ${stagedData?.negotiable === 'না' ? 'checked' : ''}><label for="negotiable-no">না</label>
                    </div>
                </div>
            `;
            ...
            priceRentHTML += `
                <div class="input-group input-inline-unit">
                    ...
                    <input type="number" id="rent-amount" placeholder="মাসিক ভাড়া" required value="${stagedData?.rentAmount || ''}">
                    <select id="rent-unit" class="unit-select" required disabled>
                        <option value="মাসিক">মাসিক (টাকায়)</option>
                    </select>
                </div>
            `;
             priceRentHTML += `
                <div class="input-group">
                    <label for="negotiable">ভাড়া আলোচনা সাপেক্ষ?</label>
                    <div class="radio-group">
                        <input type="radio" id="negotiable-yes" name="negotiable" value="হ্যাঁ" ${stagedData?.negotiable === 'হ্যাঁ' ? 'checked' : ''} required><label for="negotiable-yes">হ্যাঁ</label>
                        <input type="radio" id="negotiable-no" name="negotiable" value="না" ${stagedData?.negotiable === 'না' ? 'checked' : ''}><label for="negotiable-no">না</label>
                    </div>
        ...
        // --- সেকশন ৪: ঠিকানা ও অবস্থান ---
        let addressHTML = `
            <div class="form-section location-section">
                <h3>ঠিকানা ও অবস্থান</h3>
                <div class="input-group">
                    <label for="division">বিভাগ:</label>
                    <select id="division" required class="full-width-select">
                        <option value="">-- নির্বাচন করুন --</option>
                        ...
                        <option value="রাজশাহী" ${stagedData?.location?.division === 'রাজশাহী' ? 'selected' : ''}>রাজশাহী</option>
                        <option value="খুলনা" ${stagedData?.location?.division === 'খুলনা' ? 'selected' : ''}>খুলনা</option>
                        <option value="বরিশাল" ${stagedData?.location?.division === 'বরিশাল' ? 'selected' : ''}>বরিশাল</option>
                        <option value="সিলেট" ${stagedData?.location?.division === 'সিলেট' ? 'selected' : ''}>সিলেট</option>
                        <option value="রংপুর" ${stagedData?.location?.division === 'রংপুর' ? 'selected' : ''}>রংপুর</option>
                        <option value="ময়মনসিংহ" ${stagedData?.location?.division === 'ময়মনসিংহ' ? 'selected' : ''}>ময়মনসিংহ</option>
                    </select>
                </div>
                <div class="input-group">
                ...
                <div class="input-group">
                    ...
                    <select id="area-type-select" required class="full-width-select">
                        <option value="">-- নির্বাচন করুন --</option>
        ...
        if (stagedData && stagedMetadata) {
            ...
            stagedData.base64Images?.forEach((base64, index) => {
                ...
                handleImagePreviewDisplay(imagesInput, file, base64, true); 
            });
            imagesInput.files = tempDt.files;

            // Khotian Image (Only for sale properties)
            if (stagedData.base64Khotian && stagedMetadata.khotian) {
                const khotianInput = document.getElementById('khotian-image');
                const khotianBlob = dataURLtoBlob(stagedData.base64Khotian);
                const khotianFile = new File([khotianBlob], stagedMetadata.khotian.name, {type: stagedMetadata.khotian.type});
                
                const khotianDt = new DataTransfer();
                khotianDt.items.add(khotianFile);
                khotianInput.files = khotianDt.files;
                handleImagePreviewDisplay(khotianInput, khotianFile, stagedData.base64Khotian, true);
            }

            // Sketch Image (Only for sale properties)
            if (stagedData.base64Sketch && stagedMetadata.sketch) {
                const sketchInput = document.getElementById('sketch-image');
                const sketchBlob = dataURLtoBlob(stagedData.base64Sketch);
                const sketchFile = new File([sketchBlob], stagedMetadata.sketch.name, {type: stagedMetadata.sketch.type});
                
                const sketchDt = new DataTransfer();
                sketchDt.items.add(sketchFile);
                sketchInput.files = sketchDt.files;
                handleImagePreviewDisplay(sketchInput, sketchFile, stagedData.base64Sketch, true);
            }
        }
        
        // ইভেন্ট লিসেনারগুলো পুনরায় যোগ করা নিশ্চিত করা
        document.getElementById('images').addEventListener('change', handleImageChange);
        if(document.getElementById('khotian-image')) document.getElementById('khotian-image').addEventListener('change', handleImageChange);
        if(document.getElementById('sketch-image')) document.getElementById('sketch-image').addEventListener('change', handleImageChange);
    }

    // Function to generate sub-fields for address based on area type
        ...
        if (areaType === 'উপজেলা') {
            subFieldsHTML = `
                <div class="input-inline-group">
                    ...
                    <div class="input-group">
                        <label for="union-name">ইউনিয়ন/পৌরসভা:</label>
    ...
    // Function to handle image file selection and validation (3 max for main images)
    function handleImageChange(event) {
        const input = event.target;
        const maxFiles = input.id === 'images' ? 3 : 1;
        const previewArea = document.getElementById(input.id.replace('-image', '') + '-preview-area');
        ...
        if (files.length > maxFiles) {
            alert(`আপনি শুধুমাত্র ${maxFiles}টি ছবি আপলোড করতে পারবেন।`);
            
            // Create a new DataTransfer object with allowed files
        ...
        if (files.length > 0) {
            files.forEach(file => {
                const reader = new FileReader();
                reader.onload = (e) => handleImagePreviewDisplay(input, file, e.target.result);
                reader.readAsDataURL(file);
            });
        } else {
    ...
    function handleImagePreviewDisplay(inputElement, file, src, isStaged = false) {
        const previewAreaId = inputElement.id.replace('-image', '') + '-preview-area';
        const previewArea = document.getElementById(previewAreaId);
        const maxFiles = inputElement.id === 'images' ? 3 : 1;
        ...
        previewWrapper.className = 'preview-item-container';
        ...
        removeButton.addEventListener('click', (e) => {
            ...
            const fileIndexToRemove = currentInputFiles.findIndex(f => f.name === fileName );
    ...
    propertyForm.addEventListener('submit', async function(e) {
        ...
        submitButton.textContent = 'তথ্য প্রক্রিয়াকরণ হচ্ছে...';

        const category = document.getElementById('post-category').value;
        const type = document.getElementById('post-type')?.value;

        if (!category || !type) {
            alert('অনুগ্রহ করে পোস্টের ক্যাটাগরি ও ধরন নির্বাচন করুন।');
            submitButton.disabled = false;
            submitButton.textContent = 'এগিয়ে যান';
            return;
        }

        const getValue = (id) => document.getElementById(id)?.value.trim() || undefined;
        const getUtilityValues = () => Array.from(document.querySelectorAll('input[name="utility"]:checked')).map(el => el.value);
        ...
        if (category === 'বিক্রয়') {
            ...
            propertyData.negotiable = document.querySelector('input[name="negotiable"]:checked')?.value;
        ...
        // অতিরিক্ত ক্যাটাগরি/টাইপ-ভিত্তিক ফিল্ড যোগ করা (ভাড়া)
        } else if (category === 'ভাড়া') {
            ...
            if (type === 'বাড়ি' || type === 'ফ্লাট') {
                ...
                propertyData.kitchen = getValue('kitchen');
                if (type === 'বাড়ি') propertyData.floors = getValue('floors');
                if (type === 'ফ্লাট') propertyData.floorNo = getValue('floor-no');
            }
            if (type === 'বাড়ি') {
                 propertyData.houseArea = getValue('house-area');
                 propertyData.houseAreaUnit = getValue('house-area-unit');
        ...
        // Khotian Image
        if (khotianFile) {
            propertyData.base64Khotian = await fileToBase64(khotianFile);
        }

        // Sketch Image
        if (sketchFile) {
            propertyData.base64Sketch = await fileToBase64(sketchFile);
        }
        ...
        // Re-enable button (should not be reached due to redirect, but for safety)
        submitButton.disabled = false;
        submitButton.textContent = 'এগিয়ে যান';
    });

    // --- DOMContentLoaded লজিকের বাকি অংশ ---
    ...
    const loginLinkSidebar = document.getElementById('login-link-sidebar-menu');
    const primaryPhoneInput = document.getElementById('primary-phone');
    // New Header Elements
    const notificationButton = document.getElementById('notificationButton');
    const messageButton = document.getElementById('messageButton');
    const authCheckIndicator = document.getElementById('auth-check-indicator');
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            ...
            if (loginLinkSidebar) {
             ...
             // NEW: Load header profile image
             loadHeaderProfile(user);
             // NEW: Show/Hide new header icons
             ...
             if(authCheckIndicator) authCheckIndicator.style.display = 'none';
    ...
    // --- ✅ হেডার আইকন কার্যকারিতা (নতুন এবং কার্যক্ষম) (from post (6).js) ---

// post.js - Revised for direct Firebase Storage upload and staging URL storage
...
// --- REMOVED: fileToBase64 and dataURLtoBlob functions ---
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            if (category === 'বিক্রয়') {
                ...
                propertyData.owner = {
                    ...
                    // Note: Khotian/Sketch Base64 removed, now handled by metadata
                };
            }
    ...
    // Initial call to load staged data or setup
    // Ensure auth state is handled before calling loadStagedData
    
    // Auth State Change Logic for UI updates (পূর্বের লজিক)
    ...
    if (typeof auth !== 'undefined' && auth.onAuthStateChanged) {
        auth.onAuthStateChanged(user => {
            ...
            if (user) {
                ...
                // Fetch user data to get phone number (Dummy data for now)
                if (primaryPhoneInput) {
                    // This should be updated to fetch actual phone from Firestore profile
                    primaryPhoneInput.value = user.phoneNumber || '017XXXXXXXX'; 
                    primaryPhoneInput.disabled = true; // Disabled as it's auto-filled
                }

                // NEW: Load staged data on successful auth
                loadStagedData(); 

                // Header UI update (assuming profile image is available)
                if (headerProfileImage && defaultProfileIcon) {
                    // Placeholder for actual image loading logic
                    headerProfileImage.src = user.photoURL || 'default-profile.png'; 

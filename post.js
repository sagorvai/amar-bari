// post.js - Fixed with Client-Side Image Compression (KB size), Fast Parallel Uploads, User Profile Sync & Paid/Free Mode
const db = firebase.firestore();
const storage = firebase.storage();
const auth = firebase.auth();

// এডিট মুড ও ইউজার স্টেট ট্র্যাক করার জন্য গ্লোবাল ভেরিয়েন্বল
let isEditMode = false;
let editPostId = null;
let currentUserData = null; // ইউজারের প্রোফাইল ও পেড স্ট্যাটাস ব্যাকআপ রাখার জন্য
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    propertyForm?.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // 🌟 🎯 পেড ও ফ্রি মোড ডিটেক্ট করার লজিক
            const isPaidMember = currentUserData?.isPaidMember || currentUserData?.accountType === 'paid' || currentUserData?.subscriptionStatus === 'active' || false;
            const packageType = currentUserData?.packageType || (isPaidMember ? 'premium' : 'free');
            ...
            const propertyData = {
                ...
                // 👤 ১. ইউজার সম্পর্কিত সমস্ত তথ্য এখানে সংযুক্ত করা হলো
                user: {
                    uid: user.uid,
                    displayName: currentUserData?.displayName || user.displayName || 'Anonymous User',
                    email: currentUserData?.email || user.email || '',
                    photoURL: currentUserData?.profilePic || user.photoURL || 'assets/images/default-avatar.png',
                    phoneNumber: getValue('primary-phone')
                },

                // 💳 ২. পেড/ফ্রি মোড ডাটা
                isPaidPost: isPaidMember,
        ...
        auth.onAuthStateChanged(user => {
            ...
            if (user) {
                ...
                db.collection('users').doc(user.uid).get().then(doc => {
                    if (doc.exists) {
                        currentUserData = doc.data(); // 👤 ইউজার ও পেড মোড অবজেক্ট ব্যাকআপে রাখা হলো
                    }
                    
                    if (primaryPhoneInput && currentUserData?.phoneNumber) {
                        primaryPhoneInput.value = currentUserData.phoneNumber;
                        primaryPhoneInput.disabled = true; 
                    }
                    if (headerProfileImg && currentUserData) {
                        headerProfileImg.src = currentUserData.profilePic || user.photoURL || 'assets/images/default-avatar.png';
                    }

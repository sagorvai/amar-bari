...
onAuthStateChanged(auth, async (user) => {
    ...
    try {
        ...
        if (userDocSnap.exists()) {
            ...
            hasName = userData.name && userData.name.trim() !== "";
            hasPhoto = userData.photoURL && userData.photoURL.trim() !== "";
        } else {
            hasName = user.displayName && user.displayName.trim() !== "";
            hasPhoto = user.photoURL && user.photoURL.trim() !== "";
        }

        // প্রোফাইল অসম্পূর্ণ থাকলে কাস্টম পপআপ মোডালটি স্ক্রিনে ফুটিয়ে তোলা হবে
        if (!hasName || !hasPhoto) {
            const modal = document.getElementById('profile-modal');
            if (modal) {
                modal.style.display = 'flex';
                
                // সাবমিট বাটনটি ডিজেবল করে দেওয়া যাতে সে জোর করে ফর্ম সাবমিট করতে না পারে
                const submitBtn = document.querySelector('button[type="submit"]');
                if (submitBtn) submitBtn.disabled = true;
            }
        }
    } catch (error) {
        console.error("ভ্যালিডেশন এরর:", error);
    }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const BD_GEOGRAPHY = {
        "ঢাকা বিভাগ": {
            ...
            "ফরিদপুর": { "উপজেলা": ["ফরিদপুর সদর", "কমলগঞ্জ", "বোালমারী", "আলফাডাঙ্গা", "নগরকান্দা", "ভাঙ্গা", "সদরপুর", "চরভদ্রাসন", "সালথা"] },
